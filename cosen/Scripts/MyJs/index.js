...
function MyProductViewModel() {
    ...
    self.visitInput = ko.observable(false);//显示输入框
    self.showMsg = ko.observable(false);//显示提示信息
    //先判断总页数，然后再初始化页码
    self.init = function () {
        self.pageArr([]);
    ...
    self.writePage = function () {
        if (self.currentPage() <= 0 || self.currentPage() > self.totalPage()) {
            ...
            if (parseInt(self.currentPage()) <= 5) {
                self.pageArr([1, 2, 3, 4, 5]);
                self.tmpArr = [];
                for (var i = 1; i <= self.currentPage() ; i++) {
                    self.tmpArr.push(i);
                }
    ...
    self.search = function (type) {
        ...
        $.post("/product/search", { searchT: $("#searchT").val(), searchV: $("#searchV").val(), page: self.currentPage() }, function (data) {
            ...
            $.each(data.Infos, function (index, value) {
                ...
                self.tmpArr.push("<td>" + (value.uptime == null ? "" : value.uptime) + "</td>");
                //self.tmpArr.push("<td><button onclick='showDetails(this);' data-style_no='" + value.sty_no + "' data-price='" + value.unt_pr + "' data-name='" + value.com_nm + "'  class='btn btn-default'>明细</button></td></tr>");

                self.tmpArr.push("<td><a target='_blank' href='/product/details?styleNo=" + value.sty_no + "&price=" + value.unt_pr + "&name=" + value.com_nm + "'>明细</a></td>");
            });
    ...
    self.sltchg = function () {
        $("#searchV").val('');
        if ($("#searchT").val() === "4") {
            self.visitInput(false);
        } else {
            self.visitInput(true);
        }


    }
    //单价查询按钮
    self.search_submit = function () {
        self.currentPage(1);
        self.activeCls(self.currentPage());

        self.search("btn_click");
    }


    MyProductViewModel.searchDetails = function (style_no, price, name) {
        //alert(style_no);
        window.open('/product/details?styleNo=' + style_no + '&price=' + price + "&name=" + escape(name), "_blank");
...
function showDetails(obj) {
    MyProductViewModel.searchDetails($(obj).data("style_no"), $(obj).data("price"), $(obj).data("name"));
}
